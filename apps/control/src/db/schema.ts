...
export const shots = pgTable(
  'shots',
  {
    ...
    /**
     * **这一镜自己的地点**，覆盖场次的那个。`null` = 用 `scenes.location_id`。
     *
     * 场次与地点原本是一对一的，而**同一场里跨空间是常事**：猫眼 POV 那一镜，
     * 场次在客厅、主体在门外走廊。真机实测撞到过——渲出来的陈默「站在门外」，
     * 背景却是屋内，因为整场只有一个地点，而门外那个地点资产**压根不存在**。
     *
     * 参考图那条路还没通（P6 的 U1–U3），所以**文字是描述环境的唯一通道**：
     * 资产列不全 = 那一镜的环境靠模型猜。
     */
    locationId: uuid('location_id'),
    /**
