...
的硬底）：
- 镜头时长总和 ≈ 目标时长（±15%）。**落集级不落场级**：`scenes` 表没有
  target_duration 列，加一列等于让 S2 决定每场配多少秒，那不是 S3 的事；而
  `callShotlist()` 本来就是一次生成整集，这三条判据全是集级量。取
...
另有四条同层校验：场次数与输入不符、有台词但无说话人（S6 的 TTS 取不到
