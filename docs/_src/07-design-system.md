...
**R2 · 花钱操作必须二次确认且显示金额。** 「生成整集」这类按钮，确认弹窗上必须写清「24 个镜头，预估 $3.60」。永远不要让用户在不知道价格的情况下点下去。
...
**`<ConfirmSpend>`** — 花钱确认弹窗（R2）。必须显示：影响镜头数、预估成本、provider、当前剩余预算。确认按钮文案是「生成 24 个镜头（约 $3.60）」而不是「确定」。
