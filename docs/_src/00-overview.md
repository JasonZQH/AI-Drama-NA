...
2. **规模化的批处理与记账**：一部剧 1500–3000 个镜头，需要队列、重试、成本归集，而不是人肉点「生成」。
...
**形态标准（按北美实际口径，非中国口径）**：80–100 集 × 60–90 秒，总时长 60–100 分钟（≈一部电影），前 7–10 集免费，付费墙落在悬念点而非集尾整数位。
...
| **C8 · 素材是一等产物** | 单部剧的交付物不是 80 集正片，而是 600–1200 个产物（正片 + 素材 + 商店资产 + 联盟包）。数据模型必须把 HookConcept / Render 建成实体，而非正片的附属。 |
...
|---|---|
| **Project** | 一部剧。顶层容器。 |
| **Episode** | 一集，通常 1–2 分钟。 |
| **Scene** | 一场戏，同一时间地点的连续叙事单元。 |
| **Shot** | 一个镜头，生成的最小单位，通常 2–8 秒。 |
...
4. 生成 60 个镜头的一集，全程无需手工干预排队；失败镜头自动重试并在 UI 中标红待处理。
